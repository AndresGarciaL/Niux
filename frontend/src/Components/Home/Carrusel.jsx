...
const Carrusel = (  ) => {
    const imagenes = [
      ...
      return (
        <div className="carrusel rounded-lg">
            <Link to={imagenes[indiceActual].enlace}>
          <img src={imagenes[indiceActual].url} alt={`imagen del carrusel ${indiceActual}`} className=' hover:opacity-90' />
            </Link>
          <div className="texto-sobre-imagen">{imagenes[indiceActual].texto}</div>
          <button onClick={imagenAnterior}><GrPrevious size={50}/></button>
          <button onClick={siguienteImagen}><GrNext size={50}/></button>
        </div>
      );
    };
